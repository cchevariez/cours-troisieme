...
Afin de vérifier que deux expressions sont égales, il est **indispensable et obligatoire** de faire des calculs séparés puis de comparer les résultats obtenus pour chaque expression.

**Attention :** Il est important de distinguer deux types de questions :
- Vérifier que deux expressions sont égales pour **une valeur donnée.**
- Vérifier que deux expressions sont égales pour **toutes les valeurs.**

Voici un exemple sur un programme de calcul.
...
Pour faire court, on appelle problème ouvert, un problème où aucun chemin de résolution ne vous est imposé. Pour ce type d'exercice, il va donc falloir indiquer les étapes de votre raisonnement à votre correcteur. On peut résumer la méthode de rédaction en une seule règle : **Tous les calculs doivent être introduits.**
...
**Étape 1 : Calcul du volume de béton d'un cylindre**

Calcul du volume intérieur $V_i$

$$V_i = \pi \times r \times r \times h$$
$$V_i = \pi \times 45 \times 45 \times 50$$
$$V_i \approx 317925 \ cm^3$$

Calcul du volume extérieur $V_e$

$$V_e = \pi \times r \times r \times h$$
$$V_e = \pi \times 50.5 \times 50.5 \times 50$$
$$V_e \approx 400389 \ cm^3$$

Calcul du volume de béton $V$

$$V = V_e - V_i$$
$$V = 400389-317925$$
$$V = 82464 \ cm^3$$

Le volume de béton contenu dans un cylindre est de 82464 $cm^3$.
...
Calcul de la masse $m$ d'un cylindre de béton
$$ m = V \times \rho$$
$$m = 0{,}082464 \times 2400$$
$$m \approx 198 \ kg$$

**Étape 3 : Déterminer le nombre d'allers-retours**

On ne peut transporter que 500 kg à la fois. Or un cylindre pèse 198 kg, on ne pourra les transporter que deux par deux. Il faudra donc effectuer trois allers-retours afin de tous les transporter.
...
> |---|---|
> | Choisir un nombre | 5 |
> | Ajouter 2 à ce nombre | 7 |
> | Prendre le carré du résultat précédent | 49 |
> | Soustraire le carré du nombre de départ | 24 |
>
> 1. Vérifier en détaillant les calculs que le résultat obtenu avec le nombre 5 est bien 24.
> 2. Appliquer ce programme de calcul au nombre $-3$.
> 3. On note $x$ le nombre choisi au départ.
>     1. Montrer que l'expression du résultat en fonction de $x$ est $4x + 4$.
...
> 3. On choisit $x$ comme nombre de départ. Parmi les expressions suivantes, laquelle permet d'exprimer le résultat à l'arrivée ?
>     - Expression A : $(x + 2 \times 4)(x \times 5 - 3)$
...
> 3. Zoé prétend que son programme est « magique » car, quel que soit le nombre choisi, le résultat est toujours le double du nombre de départ. A-t-elle raison ? Justifier.
>
> **Partie B : le programme de Fred**
>
> Fred a créé le programme suivant :
> - Choisir un nombre
> - Multiplier par 4
> - Ajouter 10
> - Multiplier par 5
> - Soustraire 50
>
> 4. Démontrer que si le nombre de départ est $x$, le résultat obtenu avec le programme de Fred est $20x$.
...
> - Multiplier le résultat par 2
>
> 1. Vérifier que si on choisit le nombre $-2$, ce programme donne 0 comme résultat final.
> 2. Le programme donne 18 comme résultat final. Quel est le nombre choisi au départ ? (résolution d'équation obligatoire)
...
> - Distance Strasbourg-Marseille : 803 km
> - Formule de location de voiture choisie : 300 € + 0,25 €/km
> - Prix moyen du gazole : 1,87 €/L
> - Consommation de la voiture : 5,6 L pour 100 km
> - Coût des péages (aller-retour) : 115,80 €
>
> **Question :** leur budget de 1000 € sera-t-il suffisant pour ce voyage aller-retour ?
>
